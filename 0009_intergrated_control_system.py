...
# Scenario
# 비상 여부(is_emergency), 프라이버시 모드(is_privacy)를 불리언으로 설정.
# 배터리 잔량(battery_level)을 정수로 설정.
# 함수(master_control)에 위 3가지 값을 매개변수로 전달할 것.
# 우선순위에 따라 if-elif-else 로직을 구현할 것.
#  - 비상 상황이면 -> "CRITICAL: Emergency Stop!"
#  - 프라이버시 모드이면 -> "Privacy Active: Cameras OFF"
#  - 배터리가 20 미만이면 -> "Low Battery: Saving Mode"
#  - 모두 해당 없으면 -> "System Normal: Recording..."
