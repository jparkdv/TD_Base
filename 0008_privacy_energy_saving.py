...
# 1. [최우선] 프라이버시 모드가 켜져(True) 있다면 -> "Privacy Mode ON: Recording Disabled" 출력
# 2. [차순위] 프라이버시 모드가 꺼져(not) 있고(and), 움직임이 감지(True)되면 -> "Motion Detected: Recording Started" 출력
# 3. [그 외] 나머지 경우 -> "System Standby (Energy Saving)" 출력
